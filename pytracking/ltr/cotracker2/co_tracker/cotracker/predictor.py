...
class CoTrackerPredictor(torch.nn.Module):
    def __init__(self, checkpoint="./checkpoints/cotracker2.pth"):
        ...
        self.support_grid_size = 6 # default
        # self.support_grid_size = 8 #
        ...
        # self.use_support_grid = True
        self.use_support_grid = False # default

        print("self.upt_iters", self.upt_iters)
        print("self.use_support_grid", self.use_support_grid)
        print("self.support_grid_size", self.support_grid_size)
    ...
    def forward(
        ...
        if queries is None and grid_size == 0:
            ...
            tracks, visibilities = self._compute_sparse_tracks(
                ...
                add_support_grid=self.use_support_grid,
                grid_query_frame=grid_query_frame,
                backward_tracking=backward_tracking
            )
    ...
    def _compute_sparse_tracks(
        ...
        # print("add_support_grid", add_support_grid)
        # print("queries.shape", queries.shape)

        if eva_time:
            print("_compute_sparse_tracks forward s")
            start.record()
        ...
        train_label_last_batch,
        video=video, 
        # queries=queries, iters=6)
        queries=queries, iters=self.upt_iters)

        if eva_time:
            end.record()
            # Waits for everything to finish running
            torch.cuda.synchronize()
            print("_compute_sparse_tracks forward", start.elapsed_time(end))

        # tracks.shape torch.Size([2, 30, 4, 2])
        # visibilities.shape torch.Size([2, 30, 4])

        if eva_time:
            print("_compute_backward_tracks forward s")
            start.record()
        ...
        if eva_time:
            ...
            # Waits for everything to finish running
            torch.cuda.synchronize()
            print("_compute_backward_tracks", start.elapsed_time(end))
        ...
        # print("tracks.shape ", tracks.shape) # False wo sup
        # print("visibilities.shape ", visibilities.shape) # False sup
        # input()
        return tracks, visibilities

    def _compute_backward_tracks(self, 
                SideNetwork_D, 
                SideNetwork_U,
                # train_ltrb_target_second_emb,    
                train_label_first_batch, 
        ...
        inv_tracks, inv_visibilities, __ = self.model(
            ...
            train_label_last_batch,    
            video=inv_video, 
            # queries=inv_queries, iters=6)
            queries=inv_queries, iters=self.upt_iters)
...
class CoTrackerPredictor_ori(torch.nn.Module):
    def __init__(self, checkpoint="./checkpoints/cotracker2.pth"):
        super().__init__()
    ...
    def _compute_sparse_tracks(
        ...
        tracks, visibilities, __ = self.model.forward(video=video, queries=queries, iters=self.upt_iters)
        # tracks.shape torch.Size([2, 30, 4, 2])
        # visibilities.shape torch.Size([2, 30, 4])
    ...
    def _compute_backward_tracks(self, video, queries, tracks, visibilities):
        ...
        inv_tracks, inv_visibilities, __ = self.model(video=inv_video, queries=inv_queries, iters=upt_iters)
