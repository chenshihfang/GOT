...
def draw_circle_(rgb, coord, radius, color=(255, 0, 0), visible=True):
...
class Visualizer:
    ...
    def save_video_(self, video, filename, writer=None, step=0):
    ...
    def save_video(self, video, filename, writer=None, step=0):
        # Save video with writer if available
        if writer is not None:
            writer.add_video(
                filename,
                video.to(torch.uint8),
            ...
            # Convert video tensor to list of frames
            wide_list = list(video.unbind(1))
            wide_list = [wide[0].permute(1, 2, 0).cpu().numpy() for wide in wide_list]
            ...
            # Save each frame both to the video file, as individual images, and display them
            for i, frame in enumerate(wide_list):
                # Save frame to video
                video_writer.append_data(frame)
                
                # Save frame as an individual image
                frame_filename = os.path.join(self.save_dir, f"{filename}_frame_{i:04d}.png")
                ...
                cv2.imshow("Frame "+ str(i), frame_bgr)
            ...
            # Close the video writer and OpenCV window
            video_writer.close()
