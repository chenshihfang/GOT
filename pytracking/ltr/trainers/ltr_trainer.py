...
class LTRTrainer(BaseTrainer):
    def __init__(self, actor, loaders, optimizer, settings, lr_scheduler=None, freeze_backbone_bn_layers=False, use_GradScaler=False):
        """
        args:
    ...
    def cycle_dataset(self, loader):
        ...
        for i, data in enumerate(loader, 1):
            ...
            if loader.training:
                ...
                #########################################################
                if self.print:
                    n = 5  # Number of top gradients to display

                    # Calculate gradient norms
                    grad_norms = [(name, param.grad.norm(2).item()) for name, param in self.actor.net.named_parameters() if param.grad is not None]

                    # Sort by gradient norms in descending order
                    sorted_grad_norms = sorted(grad_norms, key=lambda x: x[1], reverse=True)

                    # Print top n gradients before clipping
                ...
                if self.print:
                    ...
                    # Print top n gradients after clipping
