...
class LTRTrainer_magav1(BaseTrainerDSA):
    def __init__(self, actor, loaders, optimizer, settings, lr_scheduler=None,
        ...
        # --- DoRA export config (can be overridden in any train_setting .py) ---
        if not hasattr(settings, "dora_save_mode"):
            settings.dora_save_mode = "none"       # "merged" | "adapters" | "both" | "none"
        if not hasattr(settings, "dora_save_every_epoch"):
            settings.dora_save_every_epoch = False
        if not hasattr(settings, "dora_save_tag"):
            settings.dora_save_tag = "MERGED_INFER_ONLY"
    ...
    def _stats_new_epoch(self):
        # Record POST-STEP LR values per param group
        for loader in self.loaders:
            if loader.training and hasattr(self, "lr_scheduler") and self.lr_scheduler is not None:
                try:
                    if hasattr(self.lr_scheduler, "get_last_lr"):
                        lr_list = list(self.lr_scheduler.get_last_lr())  # post-step values
                    else:
                        # Fallback: read current LR directly from optimizer groups
                        lr_list = [pg.get("lr", None) for pg in getattr(self, "optimizer", {}).param_groups]
                    ...
                    for i, lr in enumerate(lr_list):
                        ...
                        self.stats[loader.name][var_name].update(float(lr))
                except Exception:
                    pass
