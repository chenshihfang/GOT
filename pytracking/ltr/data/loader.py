...
def ltr_collate_stack1(batch):
    ...
    #         # Update this line to use _new_shared from FloatStorage instead
    #         storage = torch.FloatStorage._new_shared(numel)
            
    #         # Create a new tensor using the shared storage
    #         out = torch.Tensor(storage=storage).to(dtype=batch[0].dtype, device=batch[0].device)
            
    #         # Check PyTorch version for resizing
    #         if int(torch.__version__.split('.')[0]) >= 2:
    #             out.resize_(0)
    ...
    if isinstance(batch[0], torch.Tensor):
        ...
        if _check_use_shared_memory():
            ...
            # Create an uninitialized tensor with shared memory
            out = torch.empty(numel, dtype=batch[0].dtype, device=batch[0].device).share_memory_()
