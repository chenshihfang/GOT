...
class RandomHorizontalFlip(TransformBase):
    ...
    def transform_image(self, image, do_flip):
        # print("image.shape 3", image.shape)
        if do_flip:
            if torch.is_tensor(image):
                return image.flip((2,))
            return np.fliplr(image).copy()
        # print("image.shape 4", image.shape)
        return image
...
class RandomCopyPaste(TransformBase):
    """Randomly copy-paste a region within the image with a specified probability."""
    ...
    def generate_random_region(self, h, w):
        scale = random.uniform(0.8, 1.2)
        rect_w = int(random.randint(w // 9, w // 2) * scale)
        rect_h = int(random.randint(h // 9, h // 2) * scale)
        x1 = random.randint(0, w - rect_w)
        y1 = random.randint(0, h - rect_h)
        return x1, y1, x1 + rect_w, y1 + rect_h

    def transform_image(self, image, A):
        # print("image.shape 1", image.shape)
        if not self.roll():
            return image  # Skip augmentation
        h, w = image.shape[1:3]  # Adjust shape indices for tensor [C, H, W]
        x1, y1, x2, y2 = self.generate_random_region(h, w)
        patch = image[:, y1:y2, x1:x2].clone()  # Use slicing and cloning for tensors

        # Define the paste location
        px1 = random.randint(0, w - (x2 - x1))
        py1 = random.randint(0, h - (y2 - y1))
        image[:, py1:py1 + (y2 - y1), px1:px1 + (x2 - x1)] = patch
