...
def gauss_1d_PT(sz, sigma, center, end_pad=0, density=False):
    device = center.device  # Get the device from the center tensor
    k = torch.arange(-(sz - 1) / 2, (sz + 1) / 2 + end_pad, device=device).reshape(1, -1)
    gauss = torch.exp(-1.0 / (2 * sigma ** 2) * (k - center.reshape(-1, 1)) ** 2)
    if density:
        gauss /= math.sqrt(2 * math.pi) * sigma
    return gauss

def gauss_2d_PT(sz, sigma, center, end_pad=(0, 0), density=False):
    device = center.device  # Ensure all calculations are on the same device as center
    if isinstance(sigma, (float, int)):
        sigma = (sigma, sigma)
    
    gauss_x = gauss_1d_PT(sz[0].item(), sigma[0], center[:, 0], end_pad[0], density).reshape(center.shape[0], 1, -1)
    gauss_y = gauss_1d_PT(sz[1].item(), sigma[1], center[:, 1], end_pad[1], density).reshape(center.shape[0], -1, 1)
...
#     patch_size = 18
    ...
    try :
            heatmap = cv.resize(cls_score, (size,size), interpolation=cv.INTER_AREA)
    except:
            heatmap = cv.resize(cls_score.cpu().detach().numpy(), (size,size), interpolation=cv.INTER_AREA)

    heatmapshow = None
    heatmapshow = cv.normalize(heatmap, heatmapshow, alpha=0, beta=255, norm_type=cv.NORM_MINMAX, dtype=cv.CV_8U)

    heatmapshow = cv.applyColorMap(heatmapshow, cv.COLORMAP_VIRIDIS)
    
    cv.namedWindow(name, cv.WINDOW_NORMAL)
    cv.imshow(name, heatmapshow)
    cv.waitKey(1)
...
def gaussian_label_function(target_bb, sigma_factor, kernel_sz, feat_sz, image_sz, end_pad_if_even=True, density=False,
    ...
    # print("kernel_sz", kernel_sz) # (1, 1)

    # print("target_bb", target_bb) # tensor([[ 99.2149, 101.9833, 100.3580,  77.6513]])
    # print("target_bb.shape", target_bb.shape) # torch.Size([B, 4])

    # print("image_sz", image_sz) # (18., 18.)
    # print("feat_sz", feat_sz) # (288., 288.)

    image_sz = torch.Tensor(image_sz)
    feat_sz = torch.Tensor(feat_sz)

    # print("image_sz", image_sz) # tensor([288., 288.])
    # print("feat_sz", feat_sz) # tensor([18., 18.])
    ...
    # print("target_center", target_center) # tensor([[149.3939, 140.8089]])
    # print("target_center_norm", target_center_norm) # tensor([[ 0.0187, -0.0111]])
    # print("center", center) # tensor([[ 0.3371, -0.1994]])
    # print("sigma", sigma) # 0.9
    ...
    # print("label.shape", label.shape) # torch.Size([2, 18, 18])
    # input()

    # heat_show(((label[0].numpy())* 255).astype(np.uint8), "label", patch_size=18)
 
    return label



def gaussian_label_function_center(target_center, sigma_factor, kernel_sz, feat_sz, image_sz, end_pad_if_even=True, density=False, uni_bias=0):
    """Construct Gaussian label function."""

    # Convert sizes to tuples if necessary
    if isinstance(kernel_sz, (float, int)):
        kernel_sz = (kernel_sz, kernel_sz)
    if isinstance(feat_sz, (float, int)):
        feat_sz = (feat_sz, feat_sz)
