...
from ltr.models.tracking import tompnet_JEPA
...
def run(settings):
    ...
    tomp_weights_path = "ToMPL.tar"
    target_net, _ = network_loading.load_network(checkpoint=tomp_weights_path)

    net = tompnet_JEPA.tompnet50(filter_size=settings.target_filter_sz, backbone_pretrained=True, head_feat_blocks=0,
    ...
    net.feature_extractor.load_state_dict(target_net.feature_extractor.state_dict())
    net.bkMlp.load_state_dict(target_net.bkMlp.state_dict())
    net.Head_JEPA_target.load_state_dict(target_net.head.state_dict())
    ...
    for p in net.Head_JEPA_target.parameters():
        p.requires_grad_(False)

    for p in net.Head_JEPA_context.parameters():
        p.requires_grad_(True)
    ...
    actor = actors.ToMPActor_JEPAs1_vicregExpwovc400x_norm2_cexp(net=net, objective=objective, loss_weight=loss_weight)

    optimizer = optim.AdamW([
        {'params': actor.net.Head_JEPA_context.parameters(), 'lr': 1e-4},

        {'params': actor.net.JEPA_predictor_cls.parameters(), 'lr': 1e-3},
        {'params': actor.net.JEPA_predictor_breg.parameters(), 'lr': 1e-3},

        {'params': actor.net.JEPA_VICReg_Exp_c.parameters(), 'lr': 1e-4},
        {'params': actor.net.JEPA_VICReg_Exp_r.parameters(), 'lr': 1e-4},

    ], lr=2e-4, weight_decay=0.0001)

    lr_scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[25,35], gamma=0.2)
