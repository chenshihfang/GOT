...
from ltr.models.backbone.peft_dora_da3 import (
    inject_dora_into_da3,
    set_trainable_dora_params,
    collect_adapter_params,
    print_trainable_dora_params,
)
...
def run(settings):
    ...
    DORA_CFG = dict( # 921,600
        ...
        backbone_blocks=None,            # or [8,9,10,11]
        backbone_attn_only=True,
        backbone_qkvproj_only=True,      # True = qkv/proj only
        backbone_include_linear=True,
        backbone_include_conv2d=False,   # DINO blocks usually have no conv; keep False

        head_include_conv2d=False,        # DualDPT is conv-heavy -> True
        head_include_linear=False,
        head_include_aux=False,          # set True only if  really use _aux features
    ...
    # 2) enable r other trainable modules
    for m in [net.JEPA_predictor_cls, net.JEPA_predictor_breg, net.WvggtLinearCls,
            net.DA3_bkMlp, net.DiNO_VGGT_Gate, net.bkMlp, net.head]:
        for p in m.parameters():
            p.requires_grad_(True)

    # 3) inject DoRA LAST (so no need set_trainable_dora_params)
    net.feature_extractor_VGGT = inject_dora_into_da3(net.feature_extractor_VGGT, **DORA_CFG)


    # 4) print trainable DoRA parameter names after setting
    # print_trainable_dora_params(net.feature_extractor_VGGT)
    ...
    raw_groups = [
        ...
        {'params': list(_root.DA3_bkMlp.parameters()), 'lr': BASE_LRS["DA3_bkMlp"]},
