...
# CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7,8 python ltr/run_training_dsA.py tomp GOT-Edit_VGGT_378
...
# ===== Gradient clipping (r requested boolean) =====
...
def run(settings):
    ...
    dataset_train = sampler.DiMPSampler(
        [vasttrack_train, lasot_train, got10k_train, trackingnet_train, coco_train], [1, 1, 1, 1, 1],
    ...
    if os.environ.get("METRIC_PROFILE", "0") == "1":
        try:
            from ltr.profiling.profile_tomp_components import profile_tomp_components
            if dist.is_available() and dist.is_initialized():
                rank = dist.get_rank()
            else:
                rank = 0
            if rank == 0:
                print("[Metric] Profiling enabled (METRIC_PROFILE=1).")
            profile_tomp_components(
                net,
                print_latency=bool(int(os.environ.get("METRIC_LAT", "0"))),
                latency_device=os.environ.get("METRIC_LAT_DEV", "cuda"),
                B=int(os.environ.get("METRIC_B", "1")),
                S=int(os.environ.get("METRIC_S", "1")),
            )
        except Exception as e:
            print(f"[WARN] Complexity profiling skipped due to error: {e}")
    # input()
    # ----------------------------------------------------------------------
