...
from ltr.models.tracking import tompnet_JEPAp
...
def run(settings):
    ...
    JEPAc_weights_path = "got_jepa_pretrain.tar"
    JEPAc_net, _ = network_loading.load_network(checkpoint=JEPAc_weights_path)

    net = tompnet_JEPAp.tompnet50(filter_size=settings.target_filter_sz, backbone_pretrained=True, head_feat_blocks=0,
    ...
    net.head.load_state_dict(JEPAc_net.Head_JEPA_context.state_dict())
    net.head.filter_predictor.load_state_dict(JEPAc_net.Head_JEPA_context.filter_predictor.state_dict())
    net.head.classifier.load_state_dict(JEPAc_net.Head_JEPA_context.classifier.state_dict())
    ...
    optimizer = optim.AdamW([
        ...
        {'params': actor.net.bkMlp.parameters(), 'lr': 1e-4},

        {'params': actor.net.JEPA_predictor_cls.parameters(), 'lr': 2e-4},
        {'params': actor.net.JEPA_predictor_breg.parameters(), 'lr': 2e-4},

    ], lr=2e-4, weight_decay=0.0001)

    lr_scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[30,50,65], gamma=0.2)
