...
# -----------------------------------------------------------------------------
# Interpolation (safe interpolation, avoid INT_MAX overflow AND BFloat16 errors)
# -----------------------------------------------------------------------------
def custom_interpolate(
    ...
    """
    Safe interpolation implementation to avoid INT_MAX overflow in torch.nn.functional.interpolate,
    and handle missing BFloat16 kernels.
    """
    # -------------------------------------------------------------------------
    # FIX START: Handle BFloat16
    # -------------------------------------------------------------------------
    orig_dtype = x.dtype
    # If input is BFloat16, cast to Float32 because 'upsample_bilinear2d' 
    # is often not implemented for BFloat16 on many GPUs.
    ...
    if total > INT_MAX:
        ...
        out = torch.cat(outs, dim=0).contiguous()
    else:
        out = nn.functional.interpolate(x, size=size, mode=mode, align_corners=align_corners)

    # Cast back to original dtype (e.g., BFloat16) to maintain model consistency
