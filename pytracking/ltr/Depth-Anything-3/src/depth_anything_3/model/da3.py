...
class DepthAnything3Net(nn.Module):
    ...
    def forward_dpt_features(
        ...
        """
        DualDPT feature extractor (no extra upsampling to input size).

        Args:
            x: input images [B, N, 3, H, W], same as forward().
            extrinsics, intrinsics: optional camera inputs (only if you want cam_token).
            chunk_size: optional chunk length along N (sequence dim).

        Returns:
            feat_list: list of tensors, each [B, N, C_i, H_f, W_f].
        """
        # camera token logic: same as forward()
        ...
        # DualDPT feature-only path (new)
        feat_list = self.head.forward_features(
            feats,
            H,
            W,
            patch_start_idx=0,
            chunk_size=chunk_size if chunk_size is not None else 8,
        )
        return feat_list
