...
class DualDPT(nn.Module):
    ...
    def forward_features(
        ...
        """
        Feature-only path for ToMP.

        Args:
            feats: list of tokens from backbone, same as forward().
                   Each item: [B, S, T, C] (see forward()).
            H, W: original image height / width.
            patch_start_idx: first patch token index (usually 0).
            chunk_size: optional chunking along S (sequence) for memory.

        Returns:
            list of feature maps at DualDPT fusion scale, WITHOUT final upsampling:
                [f_main, aux_lvl_last, aux_lvl_prev, ...]
            Each tensor has shape [B, S, C_i, H_f, W_f].
        """
        B, S, N, C = feats[0][0].shape
        feats_flat = [feat[0].reshape(B * S, N, C) for feat in feats]

        if chunk_size is None or chunk_size >= S:
            feat_list_flat = self._forward_features_impl(feats_flat, H, W, patch_start_idx)
            # reshape back to [B, S, ...]
            feat_list = [
                f.reshape(B, S, *f.shape[1:]) for f in feat_list_flat
            ]
            return feat_list

        # chunk along S if needed
        out_chunks: List[List[torch.Tensor]] = []
        # for s0 in range(0, S, chunk_size):
        #     s1 = min(s0 + chunk_size, S)
        for s0 in range(0, B * S, chunk_size):
            s1 = min(s0 + chunk_size, B * S)
            feat_list_flat = self._forward_features_impl(
                [feat_flat[s0:s1] for feat_flat in feats_flat],
                H,
                W,
                patch_start_idx,
            )
            out_chunks.append(feat_list_flat)

        num_levels = len(out_chunks[0])
        merged: List[torch.Tensor] = []
        for lvl in range(num_levels):
            parts = [cf[lvl] for cf in out_chunks]      # each [ (s1-s0)*B, C, H_f, W_f ]
            f_all = torch.cat(parts, dim=0)             # [B*S, C, H_f, W_f]
            f_all = f_all.reshape(B, S, *f_all.shape[1:])
            merged.append(f_all)
        return merged

    def _forward_features_impl(
        self,
        feats_flat: List[torch.Tensor],
        H: int,
        W: int,
        patch_start_idx: int,
    ) -> List[torch.Tensor]:
        """
        Single-chunk feature path, no extra upsampling to (H,W).

        feats_flat: list of [B*S, N, C].
        Returns:
            list of [B*S, C_i, H_f, W_f] (same spatial scale as _fuse outputs).
        """
        BS, _, C = feats_flat[0].shape
        ...
        for stage_idx, take_idx in enumerate(self.intermediate_layer_idx):
            ...
            x = self.norm(x)
            x = x.permute(0, 2, 1).reshape(BS, C, ph, pw)  # [BS, C, ph, pw]

            x = self.projects[stage_idx](x)
            if self.pos_embed:
                x = self._add_pos_embed(x, W, H)
            x = self.resize_layers[stage_idx](x)           # internal DPT fusion scale
            resized_feats.append(x)

        # fused_main, aux_list are already at final fusion scale (no extra upsample here)
        fused_main, aux_list = self._fuse(resized_feats)
        # fused_main: [BS, F, H_f, W_f]
        # aux_list:   list of [BS, F_aux_i, H_f, W_f]

        # main + aux in reverse order so "last k" in ToMP corresponds to deeper levels first
        feat_list: List[torch.Tensor] = [fused_main] + aux_list[::-1]
        return feat_list
