...
from ltr.models.layers.normalization import InstanceL2Norm, InstanceL2Norm_reshape
...
def residual_bottleneck_reshape(feature_dim=256, num_blocks=1, l2norm=True, final_conv=False, norm_scale=1.0, out_dim=None,
                        interp_cat=False, final_relu=False, final_pool=False):
    """Construct a network block based on the Bottleneck block used in ResNet."""
    if out_dim is None:
        out_dim = feature_dim
    ...
    if final_conv:
        ...
        if final_pool:
            feat_layers.append(nn.MaxPool2d(kernel_size=3, stride=2, padding=1))
    if l2norm:
        feat_layers.append(InstanceL2Norm_reshape(scale=norm_scale))
    return nn.Sequential(*feat_layers)

def residual_bottleneck_reshape_same_dim(feature_dim=1024, num_blocks=1, l2norm=True, final_conv=False, norm_scale=1.0, out_dim=None,
                        interp_cat=False, final_relu=False, final_pool=False):
    """Construct a network block based on the Bottleneck block used in ResNet."""
    if out_dim is None:
        out_dim = feature_dim
...
def residual_bottleneck_reshape_768(feature_dim=256, num_blocks=1, l2norm=True, final_conv=False, norm_scale=1.0, out_dim=None,
                        interp_cat=False, final_relu=False, final_pool=False):
    """Construct a network block based on the Bottleneck block used in ResNet."""
    if out_dim is None:
        out_dim = feature_dim
    ...
    if interp_cat:
        feat_layers.append(InterpCat())
    for i in range(num_blocks):
        planes = 768 if i < num_blocks - 1 + int(final_conv) else out_dim // 4
        # feat_layers.append(Bottleneck(4*feature_dim, planes))
        feat_layers.append(Bottleneck(768, planes))
    if final_conv:
        # feat_layers.append(nn.Conv2d(4*feature_dim, out_dim, kernel_size=3, padding=1, bias=False))
        feat_layers.append(nn.Conv2d(768, out_dim, kernel_size=3, padding=1, bias=False))
        if final_relu:
            feat_layers.append(nn.ReLU(inplace=True))
        if final_pool:
            feat_layers.append(nn.MaxPool2d(kernel_size=3, stride=2, padding=1))
...
### SRopt
def SRopt_residual_bottleneck(feature_dim=256, num_blocks=1, l2norm=True, final_conv=False, norm_scale=1.0, out_dim=None,
                    interp_cat=False, final_relu=False, final_pool=False):
    """Construct a network block based on the Bottleneck block used in ResNet."""
    if out_dim is None:
        out_dim = feature_dim
