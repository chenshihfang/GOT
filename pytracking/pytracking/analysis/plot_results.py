...
def plot_draw_save(y, x, scores, trackers, plot_draw_styles, result_plot_path, plot_opts):
    ...
    font_size_axis = plot_opts.get('font_size_axis', 23)
    line_width = plot_opts.get('line_width', 3)
    font_size_legend = plot_opts.get('font_size_legend', 17)
    ...
    for id, id_sort in enumerate(index_sort):
        ...
        # if disp_name in "ToMP50":
        #     disp_name = r"$\mathbf{" + disp_name + "}$"

        if disp_name in "PiVOT (Ours)":
            disp_name = r"$\mathbf{" + "PiVOT" + "~" + "(Ours)" + "}$"

        if disp_name in "PiVOT-50 (Ours)":
            disp_name = r"$\mathbf{" + "PiVOT-50" + "~" + "(Ours)" + "}$"

        if disp_name in "PiVOT-L (Ours)":
            disp_name = r"$\mathbf{" + "PiVOT-L" + "~" + "(Ours)" + "}$"

        if disp_name in "PiVOT-L-22 (Ours)":
            disp_name = r"$\mathbf{" + "PiVOT-L-22" + "~" + "(Ours)" + "}$"

        if disp_name in "PiVOT-L-27 (Ours)":
            disp_name = r"$\mathbf{" + "PiVOT-L-27" + "~" + "(Ours)" + "}$"

        if disp_name in "tomp_L-18_JEPA_PT":
            disp_name = r"$\mathbf{" + "PiVOT-L-27" + "~" + "(Ours)" + "}$"
