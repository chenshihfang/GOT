...
class ToMP(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        self.label_mem = torch.zeros(self.PT_frame_num, 1, DinoPatch, DinoPatch, device='cuda')

        self.bb_mem = torch.zeros(self.PT_frame_num, 1, 1, 4, device='cuda')

        self.im_patches_mean = torch.zeros(self.PT_frame_num, 1, device='cuda')

        self.track_vis_prob = 100

        self.TFEcatAttn = True

        if self.params.TrackS == "Next":
            self.score_visflag = torch.zeros(self.PT_frame_num, 1, device='cuda')
            self.pred_tracks_test = torch.zeros(self.PT_frame_num, 1, device='cuda')
            self.pred_visibility_test = torch.zeros(self.PT_frame_num, 1, device='cuda')
        elif self.params.TrackS == "Half":
            self.score_visflag = torch.zeros(self.PT_frame_num+3, 1, device='cuda')
            self.pred_tracks_test = torch.zeros(self.PT_frame_num+3, 1, device='cuda')
            self.pred_visibility_test = torch.zeros(self.PT_frame_num+3, 1, device='cuda')
        elif self.params.TrackS == "Last":
            self.score_visflag = torch.zeros(self.PT_frame_num+7, 1, device='cuda')
            self.pred_tracks_test = torch.zeros(self.PT_frame_num+7, 1, device='cuda')
            self.pred_visibility_test = torch.zeros(self.PT_frame_num+7, 1, device='cuda')
        ...
        self.im_patches_wores_mem = torch.zeros(self.PT_frame_num, 1, 3, DinoPatch*stride, DinoPatch*stride, device='cuda')
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        backbone_feat, sample_coords, im_patches, im_patches_wores = self.extract_backbone_features(im, self.get_centered_sample_pos(),
                                                                      self.target_scale * self.params.scale_factors,
                                                                      self.img_sample_sz)

        try:
            if self.frame_num <5 and self.params.iniFramePad:
                self.im_patches_wores_mem = im_patches_wores.unsqueeze(0).repeat(8, 1, 1, 1, 1)
        except:
            pass

        if  self.frame_num > self.PT_frame_num and (self.frame_num % self.params.frames_step == 0):
            self.im_patches_wores_mem = torch.cat((self.im_patches_wores_mem[1:], im_patches_wores.unsqueeze(0)), dim=0)
        ...
        check_score_visflag = 0
        if self.params.check_score_visflag:

            if not self.PTcurq2:
                check_score_visflag = int(self.score_visflag[0])
            else:
                check_score_visflag = int(self.score_visflag[0]) * int(self.score_visflag[4]) 

        im_mem_mean = torch.mean(self.im_patches_wores_mem).unsqueeze(0).unsqueeze(1)

        self.im_patches_mean = torch.cat((self.im_patches_mean[1:, :], im_mem_mean), dim=0)

        im_patches_mean = torch.mean(self.im_patches_mean)

        if self.params.iniFramePad == "first_patch_check":
            first_patches_mean = torch.mean(self.im_patches_wores_mem[0])

            im_patches_mean = first_patches_mean

        if self.frame_num > (self.PT_frame_num+7) and check_score_visflag\
                 and (self.frame_num % self.params.frames_step == 0) and im_patches_mean != 0:

            test_imgs_all = self.im_patches_wores_mem.clone()

            test_bb_first = self.bb_mem[1].clone()

            if self.PTcurq2:
                test_bb_mid = self.bb_mem[5].clone()

            test_label_first = self.label_mem[1].unsqueeze(0).clone()
            test_label_mid = self.label_mem[(self.label_mem.shape[0] // 2)+1].unsqueeze(0).clone()

            filtered_points = None
            if self.pred_tracks_test.dim() == 4 and self.params.point_constraint:

                if self.params.TrackS == "Next":

                    last_element_test_tracks = self.pred_tracks_test[:, 1, :, :]
                    last_element_test_visibility = self.pred_visibility_test[:, 1, :]

                elif self.params.TrackS == "Half":

                    last_element_test_tracks = self.pred_tracks_test[:, 4, :, :]
                    last_element_test_visibility = self.pred_visibility_test[:, 4, :]

                elif self.params.TrackS == "Last":

                    last_element_test_tracks = self.pred_tracks_test[:, 7, :, :]
                    last_element_test_visibility = self.pred_visibility_test[:, 7, :]

                if not self.params.point_constraintall:
                    visible_points = last_element_test_tracks[last_element_test_visibility.unsqueeze(-1).expand_as(last_element_test_tracks)].view(1, -1, 2)
                else:
                    visible_points = last_element_test_tracks

                zeros = torch.zeros((visible_points.size(0), visible_points.size(1), 1), dtype=visible_points.dtype, device=visible_points.device)

                visible_points_with_zeros = torch.cat((zeros, visible_points), dim=2)

                x_min = test_bb_first[0, 0, 0]
                y_min = test_bb_first[0, 0, 1]
                width = test_bb_first[0, 0, 2]
                height = test_bb_first[0, 0, 3]
                x_max = x_min + width
                y_max = y_min + height

                x_coords = visible_points_with_zeros[0, :, 1]
                y_coords = visible_points_with_zeros[0, :, 2]
                ...
                filtered_points = visible_points_with_zeros[0, within_bounds, :].unsqueeze(0)
            ...
            if not self.PTcurq2:
                pred_tracks_test, pred_visibility_test = \
                    self.net.infer_cotracker(test_imgs_all, test_bb_first,  test_label_first, test_label_mid, filtered_points)
            else:
                pred_tracks_test, pred_visibility_test = \
                    self.net.infer_cotracker(test_imgs_all, test_bb_first, test_bb_mid,  test_label_first, test_label_mid, filtered_points)

            self.track_vis_prob = (torch.sum(pred_visibility_test[:,-1,:])/pred_visibility_test.shape[-1]).item()

            pred_tracks_test_ = pred_tracks_test.clone()
            pred_visibility_test_ = pred_visibility_test.clone()

            if self.params.ocpad:
                if self.track_vis_prob < self.params.track_vis_prob_thres:
                    self.im_patches_wores_mem[-1] = self.im_patches_wores_mem[-2]

                    if self.params.visThresIn:
                        pred_tracks_test_ = torch.cat((pred_tracks_test_[:, 1:, :, :], pred_tracks_test_[:, -1:, :, :]), dim=1)
                        pred_visibility_test_ = torch.cat((pred_visibility_test_[:, 1:, :], pred_visibility_test_[:, -1:, :]), dim=1)

            if self.params.TrackS == "Next":
                self.pred_tracks_test = pred_tracks_test_.clone()
                self.pred_visibility_test = pred_visibility_test_.clone()
            elif self.params.TrackS == "Half":

                zeros_to_add = torch.zeros(1, 3, 128, 2, device='cuda')

                self.pred_tracks_test = torch.cat((pred_tracks_test_.clone(), zeros_to_add), dim=1)

                zeros_to_add2 = torch.zeros(1, 3, 128, dtype=torch.bool, device='cuda')

                self.pred_visibility_test = torch.cat((pred_visibility_test_.clone(), zeros_to_add2), dim=1)

            elif self.params.TrackS == "Last":

                zeros_to_add = torch.zeros(1, 7, 128, 2, device='cuda')

                self.pred_tracks_test = torch.cat((pred_tracks_test_.clone(), zeros_to_add), dim=1)

                zeros_to_add2 = torch.zeros(1, 7, 128, dtype=torch.bool, device='cuda')
            ...
            test_tracks_split = self.net.gen_gaussian_label_function_center(last_element_test_tracks.contiguous(), last_element_test_visibility.contiguous())

            test_tracks_transformed = self.net.PT_prompt(test_tracks_split)

            scores_raw, bbox_preds = self.classify_target_PT(test_x, test_tracks_transformed)
        ...
        if not hasattr(self.params, 'reinitialization_threshold') or self.params.reinitialization_threshold == 0:
            if flag != 'not_found':
            ...
            reinit_threshold = self.params.get('reinitialization_threshold', 0)
            ...
            if flag != 'not_found':
                ...
                if self.target_not_found_counter > reinit_threshold:

                    self.pos = torch.Tensor([self.image_sz[0] / 2, self.image_sz[1] / 2])
        ...
        if self.frame_num > self.PT_frame_num and check_score_visflag\
                                                    and (self.frame_num % self.params.frames_step == 0):

            if self.track_vis_prob < self.params.track_vis_prob_thres:

                if self.params.mix_PT_ToMP_thres:
                    update_flag = False

        try:
            if self.frame_num <5 and self.params.iniLabelPad:
                self.label_mem = scores_raw.repeat(8, 1, 1, 1)
        except:
            pass
        ...
        if update_flag and self.params.get('update_classifier', False) and scores_raw.max() > self.params.get('conf_ths', 0.0):
            ...
            if self.frame_num % self.params.frames_step == 0:

                score_visflag = torch.tensor(1 , dtype=torch.int).unsqueeze(0).unsqueeze(0).to(scores_raw.device)

                self.bb_mem = torch.cat((self.bb_mem[1:], target_box.unsqueeze(0).unsqueeze(0).unsqueeze(0).to(scores_raw.device)), dim=0)
                self.label_mem = torch.cat((self.label_mem[1:], scores_raw), dim=0)

        else:

            if self.params.havenouptcon:
                if self.frame_num % self.params.frames_step == 0:
                    score_visflag = torch.tensor(0 , dtype=torch.int).unsqueeze(0).unsqueeze(0).to(scores_raw.device)

                    self.bb_mem = torch.cat((self.bb_mem[1:], self.bb_mem[-1].unsqueeze(0)), dim=0)
                    self.label_mem = torch.cat((self.label_mem[1:], self.label_mem[-1].unsqueeze(0)), dim=0)
        ...
        if  self.frame_num % self.params.frames_step == 0:
            self.score_visflag = torch.cat((self.score_visflag[1:], score_visflag), dim=0)
        ...
        if self.frame_num > self.PT_frame_num and check_score_visflag\
            ...
            if self.track_vis_prob < self.params.track_vis_prob_thres:
                ...
                if not self.params.mix_PT_ToMP_thres:
                    self.bb_mem = torch.cat((self.bb_mem[1:], self.bb_mem[-1].unsqueeze(0)), dim=0)
                    self.label_mem = torch.cat((self.label_mem[1:], self.label_mem[-1].unsqueeze(0)), dim=0)
                    self.im_patches_wores_mem[-1] = self.im_patches_wores_mem[-2]
    ...
    def direct_bbox_regression(self, bbox_preds, sample_coords, score_loc, scores_raw):
        shifts_x = torch.arange(
            0, self.img_sample_sz[0], step=self.params.get('feature_stride', 16),
            dtype=torch.float32
        )
        shifts_y = torch.arange(
            0, self.img_sample_sz[1], step=self.params.get('feature_stride', 16),
            dtype=torch.float32
        )
        shift_y, shift_x = torch.meshgrid(shifts_y, shifts_x)
        shift_x = shift_x.reshape(-1)
    ...
    def classify_target(self, sample_x: TensorList):
        ...
        with torch.no_grad():
            ...
            if self.params.wpcls:
                cls_weights = self.net.JEPA_predictor_cls(cls_weights)
            if self.params.wpbreg:
                bbreg_weights = self.net.JEPA_predictor_breg(bbreg_weights)
    ...
    def classify_target_PT(self, sample_x: TensorList, test_tracks_transformed):
        """Classify target by applying the DiMP filter."""
        with torch.no_grad():
            train_samples = self.training_samples[0][:self.num_stored_samples[0], ...]
            target_labels = self.target_labels[0][:self.num_stored_samples[0], ...]
            ...
            PT_promPTweight = self.net.PTrackAttentionModel(test_tracks_transformed, test_feat.unsqueeze(0), self.params.TFwPTT)
            ...
            if self.TFEcatAttn == False:
                cls_test_feat_enc = self.net.TFEcatmlp(torch.cat([cls_test_feat_enc.squeeze(0), PT_promPTweight.squeeze(0)], dim=1)).unsqueeze(0)
            else:
                cls_test_feat_enc = self.net.TFEcatmlp(cls_test_feat_enc, PT_promPTweight).unsqueeze(0)

            if self.params.wbbreg:

                if self.params.sameTFE:
                    bbreg_test_feat_enc = cls_test_feat_enc
                else:
                    if self.TFEcatAttn == False:
                        bbreg_test_feat_enc = self.net.TFEcatmlp(torch.cat([bbreg_test_feat_enc.squeeze(0), PT_promPTweight.squeeze(0)], dim=1)).unsqueeze(0)
                    else:
                        bbreg_test_feat_enc = self.net.TFEcatmlp(bbreg_test_feat_enc, PT_promPTweight).unsqueeze(0)
    ...
    def extract_backbone_features(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            im_patches_378 = self.net.resize_5d_tensor(im_patches, size = DinoPatch*14)

            im_patches_378 = im_patches_378.cuda()

            backbone_feat = OrderedDict()

            backbone_feat['layer3'] = self.net.extract_dino_features_spatial_intermediate_layers(im_patches_378)  
    ...
    def generate_init_samples(self, im: torch.Tensor) -> TensorList:
        ...
        with torch.no_grad():
            ...
            im_patches = self.net.resize_5d_tensor(im_patches, size = DinoPatch*14)

            im_patches = im_patches.cuda()

            init_backbone_feat = OrderedDict()

            init_backbone_feat['layer3'] = self.net.extract_dino_features_spatial_intermediate_layers(im_patches)
